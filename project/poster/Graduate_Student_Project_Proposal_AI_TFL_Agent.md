...
4. Incorporate the study anonymized ADaM datasets in an /adamdata folder as the data source for analysis variables and program derivations. NOTE: No study data will be copied into the AI. No patient level records will be uploaded, transmitted, or stored by the agent; testing will be done outside the agent by the student to run the agent produce SAS programs to evaluate the generated TFL outputs and log files.
...
7. The student executes the SAS program on the SAS grid and returns the resulting output files (e.g., .log, .out, .pdf). Because the AI agent has no live SAS execution environment, all iterations will follow a strict human-in-the-loop and evidence-driven cycles.
...
10. The agent identifies and diagnoses these corrections - reading the actual log text, reading rendered .out/.pdf content directly when a rendering (not syntax) problem was suspected, and reading macro source code directly rather than trusting its own assumptions about macro behavior.
11. A targeted fix is proposed and applied, syntax-checked, and handed back for another run. 
12. This cycle is repeated with each round addressing a specific, evidence-backed issue - never a speculative change without a log, output, or source-code citation to justify it.
13. Once the student is satisfied with the SAS program and output files, the student approves the generated draft.
...
## Sponsor Involvement 
A kickoff meeting should occur around the first week in May 2027 via video conference. From this point forward a student contact will be the formal point of contact with the team.
